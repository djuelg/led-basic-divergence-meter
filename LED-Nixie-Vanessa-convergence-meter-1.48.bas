...
' Random (Spielautomat) und Sekundenflip Anzeige (an / abschaltbar).
...
' 6: Datum auto Anzeige ein/aus (0)
'    0 = aus
'    1 = ein
...
'
' 3: Random (Spielautomat) und Sekundenflip ein/aus (0)
'    0 = aus
'    1 = ein
'
'******************************************************
...
105:
    ...
    if s % 10 = 4 or s % 10 = 8 and r / 10 = 0 and m = 0 and (IO.eeread(13)) = 1 then gosub 4000 ' random (Spielautomat)
    if s % 10 = 1 or s % 10 = 3 or s % 10 = 5 or s % 10 = 7 or s % 10 = 9 and m = 0 and (IO.eeread(9)) = 1 and r = 0 goto 2900 ' Partymodus
    s = IO.getrtc(0)
...
500:
    s = IO.getrtc(0)
    if s % 10 = 9 and (IO.eeread(13)) = 1 then goto 510
    n = s / 10
    p = 40
...
' VAR: n, p, s, i
700:
    if (IO.eeread(5)) = 1 and (IO.eeread(6)) = a and (IO.eeread(7)) = s then return ' alarm
    LED.irange(0, 0, 59)
    s = IO.eeread(6)
...
' Partymodus
2900:
    if (IO.eeread(5)) = 1 and (IO.eeread(6)) = a and (IO.eeread(7)) = s then goto 100 ' alarm
    i = random
...
' EEPROM INIT
8000:
    if (IO.keystate()) = 1 then IO.eewrite(0, 0xFFFF) ' Sartup INIT (Reset)
    t = IO.eeread(0)
    ...
    IO.eewrite(6, 12)            ' 6 = Alarm Zeit (Stunde)(12)
    IO.eewrite(7, 0)             ' 7 = Alarm Zeit (Minute)(0)
    IO.eewrite(8, 0)             ' 8 = Datum auto Anzeige (0)
    ...
    IO.eewrite(13, 0)            ' 13 = Random (Spielautomat) und Sekundenflip (0)
...
10160:
    ...
    if p = 3 then goto 12200 ' S3: RANDOM AND SECONDFLIP ON / OFF
...
12170:
    ...
    goto 10105
'================================================
' S3: Random (Spielautomat) und Sekundenflip ein / aus
' VAR: y,t,z,k
12200:
    gosub 9100        ' BEEP
