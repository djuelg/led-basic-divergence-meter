...
182: ' change from divergence to time display
    'if (IO.eeread(m + 1)) <> c then IO.eewrite(m + 1, c)' saves current date display to EEPROM - TODO dunno what to change
...
230:
    ...
    if z = 0 then gosub 8010 ' If first time called then recalculate divergence
    if z = 0 then gosub 5000 ' If first time called then divergence machine
