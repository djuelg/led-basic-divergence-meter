...
'    gosub 8010                   ' 18 = Calculate current birthdate
'    IO.eewrite(19, 0)            ' 19 = worldline prediction triple 1 (0)
'    IO.eewrite(20, 0)            ' 20 = worldline prediction triple 2 (0)
'
' Nixie Digit Index
...
'    data 27, 25, 1, 1, 20, 22, 1, 1, 32, 30, 27, 30, 1, 1, 37
'   Hacking to the Gate
40:  data 22, 300, 18, 150, 18, 300, 15, 150, 18, 150, 17, 300, 18, 150, 20, 600
     data 15, 150, 15, 150, 17, 300, 13, 150, 20, 300, 13, 150, 17, 150, 17, 150, 15, 600
     data 15, 300, 18, 300, 20, 300, 22, 150, 20, 300, 20, 150, 20, 150
     data 22, 300, 17, 150, 17, 300, 13, 150, 20, 150, 20, 150, 18, 600
     data 20, 300, 22, 150, 20, 150, 18, 600
'
...
99:
    ...
    j = 79 ' Initial LED location for seconds flip
...
105:
    ...
    if a = 0 and s = 0 and r = 2 then gosub 710 ' its midnight so test for new divergence
    ...
    if s % 10 = 1 or s % 10 = 3 or s % 10 = 5 or s % 10 = 7 or s % 10 = 9 and m = 0 and (IO.eeread(9)) = 1 and r = 2 goto 2900 ' party mode
    if m = 0 and r = 50 and (IO.eeread(8)) = 1 goto 180 ' date auto display
    if s % 10 = 2 or s % 10 = 6 and (IO.eeread(5)) = 1 and r = 2 gosub 800 ' show alarm time
108:
    ...
    if k = 2 and m = 2 then goto 43000 ' worldline prediction
...
180: ' change from time to date display
    if (IO.eeread(1)) <> c then IO.eewrite(1, c)' saves current time colour to EEPROM
    m = 1               ' change to date mode
    if k = 1 then x = 1 ' silence alarm when spinner pressed
    goto 190            ' done
181: ' change from date to divergence display
    if (IO.eeread(2)) <> c then IO.eewrite(2, c)' saves current date colour to EEPROM
...
230:
    ...
    if z = 0 then gosub 240 ' If first time called initialize
    gosub 700      ' SHOW DIVERGENCE - set LEDs
...
700:
    y = read 50, ((IO.eeread(18))-1) ' current birthyear
    s = IO.getrtc(5)
...
710:
    r = IO.eeread(18)
    gosub 8020
    if r <> (IO.eeread(18)) then gosub 720 else gosub 730 ' TODO REMOVE ELSE 'if birthdate id changed
    return
720:
...
' (update divergence) VAR: eeread(18): Birthday id (either 1, 4, 7 or 10)
730:
    print "Evaluating divergence prediction"
    print "predicted triple 1: ",(IO.eeread(19))
    print "predcited triple 2: ",(IO.eeread(20))
    g = read 50, ((IO.eeread(18))-1) ' current birthdate year
...
1020:
    ...
    h = (p * 8 + z) % 360 ' set "standard 1" as 8* position + counter mod 360
    goto 1090
1030:
...
3665:
    ...
    return
'............................................................
3700: ' cycling through all LEDs 0-79 in increasing colours
    LED.blackout()
...
3701: 'Pixel counter index
    ...
    data 65, 60, 66, 61, 67, 62, 68, 63, 69, 64,
    data 75, 70, 76, 71, 77, 72, 78, 73, 79, 74,
3702: 'Main loop
    for i = 1 to 79
...
8000:
    ...
    IO.eewrite(18, 0)            ' 18 = Birthdate month id (0)
    IO.eewrite(19, 0)            ' 19 = worldline prediction triple 1 (0)
    IO.eewrite(20, 0)            ' 20 = worldline prediction triple 2 (0)
    gosub 8010                   ' 18 = Calculate current birthdate
    print "EEPROM reset."
...
' VAR: w: data id of month, g: Month from data, eeread(18): Birthdate month id (either 1, 4, 7 or 10)
8010:
    ...
    if w <> (IO.eeread(18)) then IO.eewrite(18, w) ' 18 = Current birthdate month data id
    print "Birthdate id is most probably ",w
    gosub 8020
    return
'================================================
' Calculate if birthdate change
' VAR: g: Month from data, q: Day from data, eeread(18): Birthdate month id (either 1, 4, 7 or 10)
8020:
    ...
    for i = 1 to 10 step 3
        ...
        print "New Birthdate id is ",(IO.eeread(18))
    next i
    return
'================================================
...
8050:
    ...
    print "by Vanessa, partially from Doug, Domi and Jon"
    for i = 1 to 20   ' EEPROM data output
    t = IO.eeread(i)
    print "EEP[";i;"] = ";t
    next i
    return
...
10050:
    LED.irange(0, 0, 79) ' blank all the pixels
...
10120:
    ...
    goto 99            ' long press so exit
10150:
    LED.irange(0, 0, 79) ' blank out pixels
...
11050:
    LED.irange(0, 0, 79) ' blank everything
...
11150:
    LED.irange(0, 0, 79) ' blank display
...
12020:
    ...
    goto  12070
12050:
    LED.irange(0, 0, 79) ' black out LEDs
...
12120:
    ...
    goto  12170
12150:
    LED.irange(0, 0, 79) ' blank pixels
...
13320:
    gosub 8010
    gosub 710  ' check for divergence change
...
43000:
    w = -1
    y = IO.eeread(19) ' triple 1
    x = IO.eeread(20) ' triple 2
43005: ' digit loop entry
    gosub 9100        ' BEEP
    w = w + 1
    if w = 6 then w = 0 ' wraparound 0-5
    z = 0
43007:
    if w = 0 then t = y / 100
    if w = 1 then t = y / 10 % 10
    if w = 2 then t = y % 10
    if w = 3 then t = x / 100
    if w = 4 then t = x / 10 % 10
    if w = 5 then t = x % 10
    IO.setenc(t, 9, 0) ' 0-9 wraparound
43010:
    t = IO.getenc()
    if w = 0 then y = (y % 100) +  t * 100
    if w = 1 then y = (y % 10) +  t * 10 + ((y/100)*100)
    if w = 2 then y = t + ((y/10)*10)
    if w = 3 then x = (x % 100) +  t * 100
    if w = 4 then x = (x % 10) +  t * 10 + ((x/100)*100)
    if w = 5 then x = t + ((x/10)*10)
    z = 0 ' reset counter
43020:
    gosub 9000        ' GETKEY
...
43060:
    ...
    LED.show()
    z = z + 1
    if z <= 500 then goto 43010    ' time out
    goto 43320
'................................................
43300: ' save, exit
    gosub 9100        ' BEEP
